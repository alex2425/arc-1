...
| AUTH/FEATURE_TOGGLE/VERSIONS/MSAG-style reads | `src/adt/client.ts`, `src/adt/xml-parser.ts`, `src/adt/types.ts`, `src/handlers/read.ts`, `src/handlers/{schemas,tools}.ts` |
| ENHO/ENHS read (enhancement framework) | `src/adt/enhancements.ts` (collections + parsers), `src/handlers/read.ts` (`withEnhancementSource`) — the collection is release-dependent (7.50 has ONLY `enhoxh`/`enhsxs`, 7.58+ adds `enhoxhb`/`enhsxsb`), so candidates are discovery-ordered and 404/406/415 falls through. On 7.50 the object representation itself 500s (ADT had no enhancement editor before 7.53) — metadata then comes from the VIT workbench wrapper and the hook locations from `ENHINCINX` (free-SQL gated, like the TRAN lookup); class enhancements have NO servable coding there. The feed is per SUB-OBJECT and the anchor form picks the URL (include / fmodule / the include holding a FORM via the container objectstructure); class anchors are not served at all on 7.50 and the per-plug-in `elements/sourcecodeplugins` URI is not GET-able. ENHO has NO `/source/main`: the coding comes from the ENHANCED object's `{uri}/source/main/enhancements?context={MAIN PROGRAM}` feed, base64 in `<enh:source>`; a wrong/missing `context` returns an empty feed, not an error. Details: docs/research/2026-08-22-enho-adt-surface.md |
